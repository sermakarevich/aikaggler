# LB.497|Multi-Model Feature Importance Analysis

- **Author:** sf
- **Votes:** 490
- **Ref:** shodaifuruya/lb-497-multi-model-feature-importance-analysis
- **URL:** https://www.kaggle.com/code/shodaifuruya/lb-497-multi-model-feature-importance-analysis
- **Last run:** 2024-12-15 11:41:15.900000
...
```python
def process_file(filename, dirname, days_limit=21):
    # パーケットファイルを読み込み
    df = pd.read_parquet(os.path.join(dirname, filename, 'part-0.parquet'))
    
    # 計測時点からの相対時間を計算（例: 'relative_date_PCIAT' と 'time_of_day' の組み合わせ）
    df['relative_days'] = df['relative_date_PCIAT'] + df['time_of_day'] / 86400e9
    
    # 最初の60日間のデータに限定
    start_day = df['relative_days'].min()
    mask = (df['relative_days'] >= start_day) & (df['relative_days'] < start_day + days_limit)
    df = df[mask]
...
def feature_engineering(df):
    ...
    hoursDayAdd1hour = df['PreInt_EduHx-computerinternet_hoursday'] + 1 # 0だと発散するので1を足す

    # インターネット使用時間と抑うつ症状の相互作用
    df['Internet_Hours_SDS'] = hoursDayAdd1hour * df['SDS-SDS_Total_T']
     
    # インターネット使用時間が少ないことによる抑うつ症状の比率
    # df['Internet_Hours_SDS_RATE'] = df['SDS-SDS_Total_T'] / hoursDayAdd1hour

    
    # インターネット使用時間と全般的機能評価の相互作用
    # df['Internet_Hours_CGAS'] = hoursDayAdd1hour * df['CGAS-CGAS_Score']
    # インターネット使用時間と活動総合スコアの相互作用
...
featuresCols = ['Basic_Demos-Age', 'Basic_Demos-Sex',
               ...
               ,'Internet_Hours_SDS','Internet_Hours_PAQ','Internet_to_Activity_Ratio'
                ...
                ,'CGAS_PAQ_Interaction','SDS_HeartRate_Ratio'
               ]
...
featuresCols = ['Basic_Demos-Age', 'Basic_Demos-Sex',
                ...
                'BFP_BMI', 'FFMI_BFP', 'FMI_BFP', 'LST_TBW', 'BFP_BMR', 'BFP_DEE', 'BMR_Weight', 'DEE_Weight',
                'SMM_Height', 'Muscle_to_Fat', 'Hydration_Status', 'ICW_TBW','BMI_PHR','Internet_Hours_SDS','Internet_Hours_PAQ','Internet_to_Activity_Ratio'
                ,'HeartRate_Age_Ratio','HeartRate_SDS','SDS_CGAS_Ratio','SDS_PAQ_Interaction','Internet_Hours_HeartRate_Ratio'
                ,'CGAS_PAQ_Interaction','SDS_HeartRate_Ratio'
               ]
...
# meltしてlong形式へ変換し、モデル名をhueとしてまとめて描画可能
melted = all_importances.melt(id_vars='feature', var_name='model', value_name='importance')
melted = melted.dropna().sort_values('importance', ascending=False)

# 特徴量ごとの重要度の平均を算出
mean_importances = melted.groupby('feature', as_index=False)['importance'].mean()

# 平均化された特徴量重要度を並び替え
mean_importances = mean_importances.sort_values('importance', ascending=False)

plt.figure(figsize=(10,20))
sns.barplot(data=mean_importances, y='feature', x='importance', orient='h')
plt.gca().invert_yaxis()
