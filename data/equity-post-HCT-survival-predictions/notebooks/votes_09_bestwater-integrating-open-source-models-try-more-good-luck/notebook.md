# Integrating Open-Source Models:Try More,Good Luck!

- **Author:** bestwater
- **Votes:** 401
- **Ref:** bestwater/integrating-open-source-models-try-more-good-luck
- **URL:** https://www.kaggle.com/code/bestwater/integrating-open-source-models-try-more-good-luck
- **Last run:** 2025-02-15 07:39:28.830000
...
```python
!mv submission.csv submission2.csv
```
...
```python
# !cp -r /kaggle/input/refactoring-nn-pairwise-ranking-loss11/submission.csv .
# !cp -r /kaggle/input/refactoring-nn-pairwise-ranking-loss11/submission1.csv .
!mv submission.csv submission0.csv
```
...
```python
# 按照 id 列合并（假设两个文件中均包含 id 列）
merged = pd.merge(sub1, sub2, on='ID', suffixes=('_sub1', '_sub2'))

# 使用 rankdata 对 prediction 列进行排名，并融合（这里用平均排名作为示例）
merged['prediction'] = (rankdata(merged['prediction_sub1']) + rankdata(merged['prediction_sub2']))
```

```python
rankdata(merged['prediction_sub1'])
```

```python
rankdata(merged['prediction_sub2'])
```

```python
# 筛选只保留 id 和 prediction 两列
...
```python
!mv submission.csv submission3.csv
```
