# 🥇🥇MITSUI&CO baseline train&infer🥇🥇

- **Author:** yuanzhe zhou
- **Votes:** 218
- **Ref:** yuanzhezhou/mitsui-co-baseline-train-infer
- **URL:** https://www.kaggle.com/code/yuanzhezhou/mitsui-co-baseline-train-infer
- **Last run:** 2025-09-26 18:22:28.997000
...
```python
try:
    df_labels = pd.read_csv(f'{config.input_path}/train_labels.csv').set_index('date_id').tail(134)
    
    df_preds = pd.read_parquet('./submission.parquet')
    
    print('score : ', score(df_labels.reset_index().head(90), df_preds.head(90), 'date_id'))
except:
    pass
```