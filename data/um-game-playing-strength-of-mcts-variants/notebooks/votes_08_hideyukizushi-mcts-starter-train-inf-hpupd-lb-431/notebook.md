# MCTS Starter | [TRAIN+INF] HPupd [LB.431]

- **Author:** yukiZ
- **Votes:** 303
- **Ref:** hideyukizushi/mcts-starter-train-inf-hpupd-lb-431
- **URL:** https://www.kaggle.com/code/hideyukizushi/mcts-starter-train-inf-hpupd-lb-431
- **Last run:** 2024-10-01 08:43:58.650000
...
class Preprocessor():
    ...
    def train_model(self,):
        ...
        #https://www.kaggle.com/code/yunsuxiaozi/isic-2024-starter
        lgb_params={
...
#                'grow_policy'         :"Lossguide",
        ...
        xgb_params={'random_state': 2024, 'n_estimators': 2050, 
    ...
    def infer_model(self,test):
        ...
        if len(self.models):#如果有模型
            return np.mean([model.predict(test) for model in self.models],axis=0)
        else:#没模型就用均值来代替吧
            return np.array([0.04439766262399326]*len(test))
```
...
preprocessor=Preprocessor(num_folds=10,train=train)
...
```python
# df=pd.read_csv("/kaggle/input/mcts-eda-about-cv-and-lb/CV_LB.csv")
