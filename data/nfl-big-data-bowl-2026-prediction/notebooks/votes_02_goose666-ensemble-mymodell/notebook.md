...
- **Author:** goose666
- **Votes:** 346
- **Ref:** goose666/ensemble-mymodell
- **URL:** https://www.kaggle.com/code/goose666/ensemble-mymodell
- **Last run:** 2025-11-19 01:02:10.087000

---

My model train notebook is https://www.kaggle.com/code/goose666/1109mytrain0579
...
```python
# --- ENSEMBLE: Use both nfl_gru and nfl_gnn for prediction ---
import importlib
import polars as pl
import pandas as pd
import kaggle_evaluation.nfl_inference_server
...
# Import both modules (instantiates the NFLPredictor class in nfl_gnn)
# For nfl_gru, this loads the functions, but model weights are loaded lazily inside predict()
...
def predict(test: pl.DataFrame, test_input: pl.DataFrame) -> pd.DataFrame:
    # 1. Get predictions from the nfl_gru module
    # Note: This call triggers nfl_gru's lazy model loading (load_models_once) on the first batch.
    pred_gru = nfl_gru.predict(test, test_input)
    
    # 2. Get predictions from the nfl_gnn module
    # Note: This call uses nfl_gnn's global NFLPredictor instance.
    pred_gnn = nfl_gnn.predict(test, test_input)
    pred_mymodel = nfl_mymodel.predict(test, test_input)

    # 3. Get predictions from the nfl_mymodel module (添加这行)
    pred_mymodel = nfl_mymodel.predict(test, test_input)
    
    # Ensure both are Pandas DataFrames for averaging (nfl_gru returns Polars, nfl_gnn returns Pandas)
    ...
    # Average predictions
    # This assumes the order of players/frames in pred_gru and pred_gnn is identical,
    # which is required and guaranteed by the API's input 'test' DataFrame structure.
    pred_ensemble = pred_gru[['x', 'y']].values * 0.34 + pred_gnn[['x', 'y']].values * 0.33 + pred_mymodel[['x', 'y']].values * 0.33
